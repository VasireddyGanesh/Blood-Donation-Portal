...
if (isset($_POST['submit'])) {
        $actualcode = $_SESSION['activatation_code'];
        $enteredcode=$_POST['activation-code'];
        if($actualcode==$enteredcode){
            $fname=$_SESSION['sess_user'];
            $email=$_SESSION['email'];
            $passwrd=$_SESSION['password'];
            $encrypted_passwrd=md5($passwrd);
            $sql = "INSERT INTO `user` (`name`,`email`,`passwrd`,`activitation_code`) VALUES ('$fname','$email','$encrypted_passwrd','$actualcode')";
...
<body>
    ...
    <div id="container">
        <div id="form-container">
            <form id="side" method="POST" action="account_confirmation.php">
